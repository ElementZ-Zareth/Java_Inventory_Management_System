...
public class Modify_productController{
    ...
    // Controller for modified product so we can instantiate variables
    public Modify_productController(int id, String name, int inv, double price, int max, int min, ArrayList<Part> parts){
        id_T = id;
        name_T = name;
        inv_T = inv;
        price_T = price;
    ...
    int uniqueID = 1 + random_Number.nextInt(50);
    ...
    public void save_product(ActionEvent event) throws IOException {
        if (!canSave()){
            ...
            Inventory.updateProduct(id, inv, price, max, min, name, parts);
